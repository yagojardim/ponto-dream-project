...
export const ONBOARDING_TIPS: Record<string, OnboardingTip> = {
  ...
  calendar: {
    ...
    guide: [
      ...
      { heading: 'Criar evento', text: 'Em "+ Criar" você registra um evento novo. Obrigatório é só o título; o tipo (Reunião, Daily, Planning…) já define a cor. Depois vêm o período (dia inteiro ou data + horário de início/fim), os convidados, o link de videochamada e a descrição. Os convidados são buscados entre os membros do tenant. Use "Mais opções" para abrir os campos avançados: local, cor, vínculo com uma demanda (work item) e lembrete.', image: '/help/calendario--criar-mais-opcoes.png', imageAlt: 'Criação de evento com mais opções' },
      { heading: 'Seletor de Sprint', text: 'O seletor de Sprint na barra de topo tem uma função só: escolher de qual sprint as cerimônias serão geradas. Ele é o alvo do botão "Gerar cerimônias da sprint" ao lado — não filtra o que aparece no calendário. Só é exibido para quem tem a permissão Gerenciar Sprint e existe pelo menos uma sprint aberta.' },
      { heading: 'Gerar cerimônias da sprint', text: 'O botão "Gerar cerimônias da sprint" abre um painel onde você configura cada cerimônia (Daily, Planning, Review, Retro…): ativa/desativa, escolhe os dias da semana, a ocorrência (todas, primeira ou última) e o horário. Ao gerar, os eventos entram no calendário já posicionados — e cerimônias já existentes na mesma data/horário não são duplicadas.', image: '/help/02-calendario--gerar-cerimonias.png', imageAlt: 'Gerar cerimônias da sprint' },
      { heading: 'Integrar sua agenda Google', text: 'Em "Integrar agenda" você conecta sua conta Google: clique em "Conectar", autorize no login do Google e a sincronização roda automaticamente. Depois de conectado surgem "Sincronizar agora" e "Desconectar". Eventos vindos do Google aparecem com o selo "G" e são somente leitura aqui. Teams e Outlook estão em construção.', image: '/help/02-calendario--integrar-agenda.png', imageAlt: 'Integração com a agenda Google' },
    ],
  },
  'projects-list': {
    title: 'Projetos',
