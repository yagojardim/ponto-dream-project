...
export interface WorkloadEntry {
  ...
  active: number
  points: number
  /** Carga da pessoa por projeto (itens ativos e pontos). */
  byProject: { projectId: string; active: number; points: number }[]
}
...
export async function fetchDashboardAggregates(projectIds?: string[]): Promise<DashboardAggregates> {
  ...
  const workload: WorkloadEntry[] = profileRows
    .map(p => {
      ...
      const byPid = new Map<string, { active: number; points: number }>()
      for (const i of rows) {
        const cur = byPid.get(i.project_id) ?? { active: 0, points: 0 }
        byPid.set(i.project_id, { active: cur.active + 1, points: cur.points + Number(i.story_points ?? 0) })
      }
      return {
        profileId: p.id,
        name: p.name,
        ...
        byProject: [...byPid.entries()].map(([projectId, v]) => ({ projectId, ...v })).sort((a, b) => b.points - a.points),
      }
    })
    .filter(w => w.active > 0)
