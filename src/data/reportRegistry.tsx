...
export function BurndownChart({ variant = 'full', data: explicit }: ChartProps) {
  ...
  return (
    <ChartFrame data={data} loading={loading} error={error} height={th ? 60 : 180}
      ...
      {() => {
        const b = bd!
        // Multi-projeto (nível Jira): 1 burndown por projeto quando há mais de uma sprint ativa.
        if (!th && data && data.sprintProgress.sprints.length > 1) {
          return <SprintBurndownGrid data={data} />
        }
...
interface VelItem { name: string; color: string; completed: number[]; committed: number[] }

/** Mini velocity de um projeto: comprometido (contorno) × entregue (preenchido) + média. */
function VelocityMini({ item }: { item: VelItem }) {
  const K = item.completed.length
  const rel = Array.from({ length: K }, (_, i) => (i === K - 1 ? 'atual' : `-${K - 1 - i}`))
  const W = 240, H = 108, PAD = { top: 10, right: 8, bottom: 16, left: 20 }
  const cw = W - PAD.left - PAD.right, ch = H - PAD.top - PAD.bottom
  const max = Math.max(1, ...item.committed, ...item.completed) * 1.15
  const slot = cw / Math.max(1, K), bw = slot * 0.5
  const x = (i: number) => PAD.left + i * slot + slot * 0.25
  const y = (v: number) => PAD.top + ch - (v / max) * ch
  const axisY = PAD.top + ch
  const nz = item.completed.filter(v => v > 0)
      ...
      <div style={{ padding: `${px(2)} ${px(8)} ${px(8)}` }}>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: 'block' }}>
          ...
          {rel.map((l, i) => {
            ...
            return (
              <g key={l + i}>
                <rect x={bx} y={y(item.committed[i])} width={bw} height={axisY - y(item.committed[i])} rx={2} fill="none" stroke={item.color} strokeWidth={1} opacity={0.6} />
                <rect x={bx} y={y(item.completed[i])} width={bw} height={axisY - y(item.completed[i])} rx={2} fill={item.color} opacity={0.85} />
                {item.completed[i] > 0 && <text x={bx + bw / 2} y={y(item.completed[i]) - 2} textAnchor="middle" fontSize={7} fill={T.text2}>{item.completed[i]}</text>}
                <text x={bx + bw / 2} y={H - 4} textAnchor="middle" fontSize={7} fill={T.text3}>{l}</text>
              </g>
            )
          })}
        </svg>
...
/** Grid de velocity por projeto (small-multiples) + resumo agregado. */
function VelocityGrid({ items }: { items: VelItem[] }) {
  const avgOf = (arr: number[]) => { const nz = arr.filter(v => v > 0); return nz.length ? Math.round(nz.reduce((a, b) => a + b, 0) / nz.length) : 0 }
  const avgAll = items.length ? Math.round(items.reduce((a, it) => a + avgOf(it.completed), 0) / items.length) : 0
  const totC = items.reduce((a, it) => a + it.committed.reduce((x, y) => x + y, 0), 0)
  ...
  return (
    <ChartFrame data={data} loading={loading} error={error} height={th ? 60 : 140}
      ...
      {() => {
        ...
        // Small-multiples por projeto (comprometido × entregue) sempre que há dados por projeto.
        if (!th && byP.length >= 1) {
          const info = new Map(liveProjects().map(p => [p.id, p]))
          const items = byP.map(b => ({ name: info.get(b.projectId)?.name ?? 'Projeto', color: info.get(b.projectId)?.color ?? T.accent, completed: b.completed, committed: b.committed }))
          return <VelocityGrid items={items} />
        }
...
function miniTiles(items: { l: string; v: string | number; c?: string; s?: string }[]) {
  // Compacto: os cards internos não competem com o gráfico (foco no gráfico).
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: px(6) }}>
      {items.map(it => (
        <div key={it.l} style={{ flex: '1 1 auto', minWidth: px(78), background: T.bgSurface2, border: `1px solid ${T.border}`, borderRadius: 8, padding: `${px(5)} ${px(9)}` }}>
          <div style={{ fontSize: 9, color: T.text3, lineHeight: 1.2 }}>{it.l}</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: px(5) }}>
            <span style={{ fontSize: 14, fontWeight: 750, color: it.c ?? T.text1, lineHeight: 1.15 }}>{it.v}</span>
            {it.s && <span style={{ fontSize: 9, color: T.text3 }}>{it.s}</span>}
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <ChartFrame data={data} loading={loading} error={error} height={th ? 60 : 160}
      ...
      {() => {
        ...
        if (th) return chart
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: px(8), height: '100%', minHeight: 0 }}>
            <div style={{ flex: '1 1 auto', minHeight: px(150) }}>{chart}</div>
  ...
  return (
    <ChartFrame data={data} loading={loading} error={error} height={th ? 80 : 150}
      ...
      {() => {
        ...
        const radar = (
          <svg viewBox="0 0 176 156" preserveAspectRatio="xMidYMid meet" style={th ? { display: 'block', width: '100%' } : { display: 'block', width: 168, height: 150, maxWidth: '100%', flexShrink: 0 }}>
...
/** Burndown multi-projeto para o card do board: resumo + 1 burndown por projeto. */
function SprintBurndownGrid({ data }: { data: ReportsData }) {
  const sp = data.sprintProgress
  const single = sp.sprints.length === 1
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: px(10), height: '100%', minHeight: 0 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: px(14), padding: `${px(8)} ${px(10)}`, background: T.bgSurface2, border: `1px solid ${T.border}`, borderRadius: 10 }}>
        <SummaryStat label="Conclusão média" value={`${sp.summary.avgPct}%`} />
        <SummaryStat label="Menor prazo" value={sp.summary.daysLeftMin != null ? `${sp.summary.daysLeftMin}d` : '—'} color={sp.summary.daysLeftMin != null && sp.summary.daysLeftMin <= 2 ? T.warn : undefined} />
        <SummaryStat label="No prazo" value={String(sp.summary.onTrack)} color={T.success} />
        <SummaryStat label="Em risco" value={String(sp.summary.atRisk)} color={sp.summary.atRisk > 0 ? T.warn : T.text1} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: single ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: px(12) }}>
        {sp.sprints.map(b => <ProjectBurndownCard key={b.projectId + b.sprintName} b={b} single={single} />)}
      </div>
    </div>
  )
}
...
function VelocityAnalysisView({ data }: { data: ReportsData }) {
  const info = new Map(liveProjects().map(p => [p.id, p]))
  const items = data.velocity.byProject.map(b => ({ id: b.projectId, name: info.get(b.projectId)?.name ?? 'Projeto', color: info.get(b.projectId)?.color ?? T.accent, completed: b.completed, committed: b.committed }))
  const [sel, toggle] = useScopeSel(items.map(i => i.id))
  const shown = items.filter(i => sel.has(i.id))
  const use = shown.length ? shown : items
  if (items.length === 0) {
  ...
  const prevOf = (u: { committed: number[]; completed: number[] }) => { const c = u.committed.reduce((a, b) => a + b, 0); return c ? Math.round((u.completed.reduce((a, b) => a + b, 0) / c) * 100) : 0 }
  const note = sel.size === 1
    ? `${use[0].name}: média de ${avgOf(use[0].completed)} pts/sprint e previsibilidade de ${prevOf(use[0])}% (entregue ÷ comprometido).`
    : `Comparando ${use.length} projetos — previsibilidade orienta o quanto comprometer por sprint sem estourar.`
  return (
    <>
      <ScopeChips items={items} selected={sel} onToggle={toggle} />
      <VelocityGrid items={use.map(u => ({ name: u.name, color: u.color, completed: u.completed, committed: u.committed }))} />
      <div style={{ marginTop: px(8) }}><AnalysisNote text={note} /></div>
    </>
  )
}
...
function WorkloadAnalysisView({ data }: { data: ReportsData }) {
  const info = new Map(liveProjects().map(p => [p.id, p]))
  const people = data.workload
  const projItems = [...new Set(people.flatMap(w => w.byProject.map(b => b.projectId)))].map(pid => ({ id: pid, name: info.get(pid)?.name ?? 'Projeto', color: info.get(pid)?.color ?? T.accent }))
  const [sel, toggle] = useScopeSel(projItems.map(p => p.id))
  const [open, setOpen] = useState<string | null>(null)
  if (people.length === 0) {
    return <div style={{ fontSize: 12, color: T.text3, border: `1px dashed ${T.border}`, borderRadius: 10, padding: '24px 16px', textAlign: 'center' }}>Nenhuma demanda ativa atribuída.</div>
  }
  const rows = people.map(w => ({ w, bp: w.byProject.filter(b => sel.has(b.projectId)) })).filter(x => x.bp.length > 0)
  const maxTotal = Math.max(1, ...rows.map(r => r.bp.reduce((a, b) => a + b.pts, 0)))
  return (
    <>
      <ScopeChips items={projItems} selected={sel} onToggle={toggle} />
      <div style={{ fontSize: 11, color: T.text3, marginBottom: px(8) }}>Carga ativa (pontos em aberto) por pessoa — clique para ver por projeto.</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: px(7) }}>
        {rows.map(({ w, bp }) => {
          const total = bp.reduce((a, b) => a + b.pts, 0)
          const color = total >= 12 ? T.crit : total >= 9 ? T.warn : T.success
          const opened = open === w.fullName
          const maxPts = Math.max(1, ...bp.map(b => b.pts))
          return (
            <div key={w.fullName}>
              <div onClick={() => setOpen(opened ? null : w.fullName)} style={{ display: 'flex', alignItems: 'center', gap: px(8), cursor: 'pointer' }}>
                <span style={{ width: 22, height: 22, borderRadius: '50%', background: T.bgSurface2, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: T.text2, flexShrink: 0 }}>{w.name}</span>
                <span style={{ flex: 1, fontSize: 12, color: T.text1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{w.fullName}</span>
                <div style={{ width: 120, height: 10, background: T.bgSurface2, borderRadius: 5, overflow: 'hidden' }}>
                  <div style={{ width: `${Math.min(100, (total / maxTotal) * 100)}%`, height: '100%', background: color, opacity: 0.85 }} />
                </div>
                <span style={{ width: 42, textAlign: 'right', fontWeight: 600, color, fontSize: 12 }}>{total}pt</span>
              </div>
              {opened && (
                <div style={{ padding: '6px 0 6px 30px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                  {bp.map(b => (
                    <div key={b.projectId} style={{ display: 'flex', alignItems: 'center', gap: px(8), fontSize: 11 }}>
                      <span style={{ width: 110, color: T.text2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{info.get(b.projectId)?.name ?? 'Projeto'}</span>
                      <div style={{ flex: 1, height: 8, background: T.bgSurface2, borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ width: `${Math.min(100, (b.pts / maxPts) * 100)}%`, height: '100%', background: info.get(b.projectId)?.color ?? T.accent, opacity: 0.85 }} />
                      </div>
                      <span style={{ width: 60, textAlign: 'right', color: T.text2 }}>{b.pts}pt · {b.active}</span>
                    </div>
                  ))}
                </div>
              )}
...
const MGMT_HEADERS: Record<string, { title: string; subtitle: string }> = {
  ...
  workload: { title: 'Workload por Pessoa', subtitle: 'Carga por pessoa e por projeto' },
}
...
export function ReportChartModal({ reportId, onClose, onNav }: {
  ...
  } else if (isMgmt && data && reportId === 'workload') {
    body = <WorkloadAnalysisView data={data} />
  } else {
    body = <Chart />
  }
