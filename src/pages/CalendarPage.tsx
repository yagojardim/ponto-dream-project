...
import {
  ...
  DEFAULT_TENANT_IDimport { SprintCeremoniesModal } from '@/components/SprintCeremoniesModal'
import { useState, useRef, useEffect, useCallback, type CSSProperties } from 'react'
import { T } from '@/components/ds/tokens'
import { listDeadlines, type DeadlineItem } from '@/data/db/calendar'
import { type CalendarEvent } from '@/data/calendarEvents'
...
function EventComposer({ initial, onSave, onClose }: ComposerProps) {
  ...
  // Convidados = todos os membros configurados no tenant (profiles), não as personas de inspeção.
  useEffect(() => { void getMembers().then(setMembers) }, [])

  const guestSuggestions = guestQ.length > 0
    ? members.filter(m =>
        m.email.length > 0 &&
        (m.name.toLowerCase().includes(guestQ.toLowerCase()) ||
         m.email.toLowerCase().includes(guestQ.toLowerCase())) &&
        !guests.find(g => g.email === m.email)
      ).slice(0, 6)
    : []

  function addGuest(m: { name: string; email: string }) {
    setGuests(gs => [...gs, { name: m.name || m.email, email: m.email }]); setGuestQ('')
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {guestSuggestions.length > 0 && (
                <div style={{
                  ...
                  {guestSuggestions.map(m => {
                    const initials = (m.name.trim().split(/\s+/).map(w => w[0]).slice(0, 2).join('') || m.email[0]).toUpperCase()
                    return (
                    <button key={m.id} onClick={() => addGuest(m)} style={{
                    ...
                    onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent' }}>
                      <div style={{ width: 24, height: 24, borderRadius: '50%', background: T.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>{initials}</div>
                      <div>
                        <div style={{ fontSize: 12, color: T.text1 }}>{m.name || m.email}</div>
                        <div style={{ fontSize: 10, color: T.text3 }}>{m.email}</div>
                      </div>
                    </button>
                    )
                  })}
          ...
          {!expanded && (
            <button data-tour="cc-more" onClick={() => setExpanded(true)} style={{ fontSize: 12, color: T.accent, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0 }}>
              ＋ Mais opções (local, cor, lembrete…)
            </button>
          )}
